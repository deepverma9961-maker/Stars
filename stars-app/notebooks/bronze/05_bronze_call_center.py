...
for i in range(0, len(rows), CHUNK):
    ...
    chunk_df.write.format("delta").mode(mode).saveAsTable(f"{CATALOG}.bronze.bronze_call_center_raw")

total = spark.table(f"{CATALOG}.bronze.bronze_call_center_raw").count()
